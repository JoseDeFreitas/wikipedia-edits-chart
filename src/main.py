...
@app.get("/{username}")
async def get_user(
    app_response: Response, username: str, language: str,
    year: str, appearance: str = "light"
    ):
    app_response.headers["Content-Type"] = "image/svg+xml"
    ...
    return Response(content=edit_data, media_type="image/svg+xml")
